  ...
  <div id="tags-view-container" class="tags-view-container" :class="{ 'tags-view-container--chrome': tagsViewStyle === 'chrome' }">
    ...
    <scroll-pane ref="scrollPaneRef" class="tags-view-wrapper" @scroll="handleScroll" @update-arrows="updateArrowState">
      <router-link
        ...
        :key="tag.path"
        :data-path="tag.path"
        :class="{ 'active': isActive(tag), 'has-icon': tagsIcon, 'is-before-active': isBeforeActive(index) }"
      ...
      >
        <svg-icon v-if="tagsIcon && tag.meta && tag.meta.icon && tag.meta.icon !== '#'" :icon-class="tag.meta.icon" class="tags-view-icon" />
...
function isBeforeActive(index) {
  const nextTag = visitedViews.value[index + 1]
  return nextTag ? isActive(nextTag) : false
}

function tagActiveStyle(tag) {
  if (!isActive(tag)) return {}
  return {
    '--tags-active-color': theme.value
  }
}
...
.tags-view-container {
  ...
  $btn-width: 28px;
  $btn-color: #64748b;
  $btn-hover-bg: rgba(255, 255, 255, 0.86);
  ...
  .tags-view-wrapper {
    ...
    .tags-view-item {
      ...
      .tags-divider {
        ...
        width: 1px;
        height: 18px;
        background: rgba(95, 111, 131, 0.32);
        transform: translateY(-50%);
        pointer-events: none;
      }

      &.active .tags-divider,
      &.is-before-active .tags-divider {
        opacity: 0;
      }
    }
  }
  ...
  &:not(.tags-view-container--chrome) .tags-view-wrapper .tags-view-item.active {
    ...
    height: 35px;
    color: var(--tags-active-color, var(--el-color-primary, #409eff));
    ...
    &::before {
      left: -13px;
      background: radial-gradient(circle at 0 0, transparent 12px, rgb(243, 243, 243) 12px 13px, rgb(255, 255, 255) 13px);
    }

    &::after {
      right: -13px;
      background: radial-gradient(circle at 100% 0, transparent 12px, rgb(243, 243, 243) 12px 13px, rgb(255, 255, 255) 13px);
    }
  }
  ...
  &.tags-view-container--chrome {
    --chrome-strip-bg: rgb(244, 244, 244);
    --chrome-strip-border: transparent;
    --chrome-tab-active-bg: rgb(255, 255, 255);
    --chrome-tab-text: var(--el-text-color-regular, #606266);
    --chrome-tab-text-active: var(--tags-active-color, var(--el-color-primary, #409eff));
    --chrome-wing-r: 13px;
    ...
    .tags-view-wrapper {
      .tags-view-item {
        ...
        transition: background 0.14s ease, color 0.14s ease, box-shadow 0.14s ease, border-radius 0.14s ease;
        ...
        .tags-divider {
          right: 0;
          height: 18px;
          background: rgba(95, 111, 131, 0.34);
        }
        ...
        &.active {
          ...
          &::before {
            border-radius: 0 !important;
            box-shadow: none;
            background: radial-gradient(circle at 0 0, transparent 12px, rgb(243, 243, 243) 12px 13px, var(--chrome-tab-active-bg) 13px) !important;
          }

          &::after {
            border-radius: 0 !important;
            box-shadow: none;
            background: radial-gradient(circle at 100% 0, transparent 12px, rgb(243, 243, 243) 12px 13px, var(--chrome-tab-active-bg) 13px) !important;
          }
        }
      }
    }
